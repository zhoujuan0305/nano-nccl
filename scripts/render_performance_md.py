...
def render(doc: dict) -> str:
    ...
    out.append(
        ...
        "Two-host RDMA runs use nano `--transport rdma` with `NANO_NCCL_RDMA_USE_WRITE=1` "
        "(WRITE+CTS; local edges P2P when NVLink peer access is available, otherwise SHM; "
        "dedicated per-proxy threads by default) and NCCL IB/RoCE with `NCCL_NET_GDR_LEVEL=0`."
    )
    out.append("")
    out.append(
        ...
        "RDMA SEND/WriteCts always post from the registered mapped FIFO (no host bounce; "
        "visibility via publisher `fence.acq_rel.sys` + relaxed `send_tail` store, matching NCCL Simple postPeer)."
    )
    out.append("")
