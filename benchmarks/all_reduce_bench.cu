...
int main(int argc, char** argv) {
    ...
    std::printf("# algo %s nGpus %d warmup iters: %d iters: %d validation: 1\n",
                config.algo.c_str(), nano_nccl::kRanks,
                config.warmup_iters, config.iters);
    std::printf("# %14s %12s %12s %10s %10s %10s %8s %12s\n", "algo",
                "size(B)", "count", "time(us)", "algbw", "busbw", "#wrong",
                "max_abs");
    for (const auto& result : results) {
        std::printf("%14s %12zu %12zu %10.2f %10.2f %10.2f %8d %12.6g\n",
                    result.algo.c_str(), result.bytes, result.count,
                    result.time_us, result.algbw, result.busbw, result.wrong,
